...
func replaceDigits(input: String) -> String {
  ...
  if let first = ranges.first {
    // check if there is a digit character before the first spelled out digit and don't replace if there is
    if let before = input.index(before: first.range.lowerBound), input[before].isNumber {
      returned = input
    } else {
