...
func replaceDigits(input: String) -> String {
  ...
  if let first = ranges.first {
    // check if there is a digit character anywhere before the first spelled out digit in the input string and don't replace if there is
    let before = input.index(before: first.range.lowerBound)

    for char in input[..<before] where char.isNumber {
      returned = input
      break
    }
